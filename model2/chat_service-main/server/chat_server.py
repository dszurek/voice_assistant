...
elif config.model_id == "NTQAI/Nxcode-CQ-7B-orpo":  # Cf. https://huggingface.co/NTQAI/Nxcode-CQ-7B-orpo
    model = AutoModelForCausalLM.from_pretrained(config.model_id, torch_dtype=torch.float16, device_map=config.device)
    tokenizer = AutoTokenizer.from_pretrained(config.model_id)
else:
...
def reply_to_prompt():
    ...
    elif config.model_id == "NTQAI/Nxcode-CQ-7B-orpo":
        messages = [
            {"role": "user", "content": f"{config.model_instructions}\n{prompt}"}
        ]
        model_inputs = tokenizer.apply_chat_template(messages, add_generation_prompt=True,
                                                     return_tensors="pt").to(config.device)
        outputs = model.generate(model_inputs, max_new_tokens=config.max_new_tokens, do_sample=False,
                                 top_k=50, num_return_sequences=1, eos_token_id=tokenizer.eos_token_id)
        res = tokenizer.decode(outputs[0][len(model_inputs[0]):], skip_special_tokens=True)
        return res
    else:
